...
| `courses/` | **Where generated courses land** — one subfolder per course (`courses/<name>/`), each holding the content + `COURSE_BRIEF.md` + `SOURCES.md` + a **`FEEDBACK.md`** (empty template; gathers critiques / things-to-fix for *that* course) + a **`COURSE_REPORT.md`** (the final graded scorecard from `course-evaluator`) + its `.claude` residue. |
